export const extractPropertiesFromJsonObject = (jsonObject: { [key: string]: any }) => {
  const formBody = Object.keys(jsonObject).reduce<Record<string, any>>((previousValue, currentValue) => {
    ...
    if (value instanceof Array) {
      const arrayItem = value.reduce((accArray, valueArray) => {
        if (valueArray instanceof Object) {
          accArray.push(`${encodeURIComponent(currentValue)}=${encodeURIComponent(JSON.stringify(valueArray))}`)
        } else {
    ...
    if (value instanceof Object) {
      previousValue.push(`${encodeURIComponent(currentValue)}=${encodeURIComponent(JSON.stringify(value))}`)
    } else {
